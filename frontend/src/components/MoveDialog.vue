...
const doneInfo = ref<{files_done: number; errors: Array<{file: string; reason: string}>} | null>(null)
const opError = ref<{code: string; message: string} | null>(null)

let wsHandle: OperationWsHandle<MoveEvents> | null = null
...
<template>
    <ModalDialog :title="'Move to: ' + destination">
        <template v-if="!done && !opError">
            ...
            <div class="progress-stats">
                {{ progress.files_done }} files moved
            </div>
        ...
        <template v-else-if="done && doneInfo">
            <div class="done-msg">Done: {{ doneInfo.files_done }} files moved</div>
