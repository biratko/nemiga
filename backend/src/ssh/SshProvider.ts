...
import {SshAdapter} from './SshAdapter.js'
...
export class SshProvider implements FileSystemProvider {
    ...
    constructor(sessionId: string, params: SshConnectionParams, options?: SshProviderOptions) {
    ...
    async connect(params: SshConnectionParams): Promise<void> {
    ...
    /** Mutex-wrapped adapter call — serializes all SSH commands per-connection */
    ...
    get pathPrefix(): string {
        return `ssh://${this.sessionId}@${this.host}`
    }
    ...
    private async copyRecursive(src: string, dest: string, ctx: OperationContext): Promise<void> {
        ...
        if (entries.length === 0 || isSelfListing) {
            ...
            const tmpPath = path.join(os.tmpdir(), `nemiga-ssh-${Date.now()}-${Math.random().toString(36).slice(2)}`)
            try {
